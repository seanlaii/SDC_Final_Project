...
def callback(odom_data):
  with open(path, 'ab') as csvfile:
    ...
    n = "{:0>9d}".format(ntime)
    writer.writerow([str(time)+"."+n, odom_data.pose.pose.position.x, odom_data.pose.pose.position.y, odom_data.pose.pose.position.z])
...
if __name__ == '__main__':
  ...
  path = rospack.get_path('generate_answer') + "/../localization_ground_truth/icp2.csv"
