...
icp_local::icp_local(){
	...
	downsample_map.setLeafSize (0.7, 0.7, 0.7);
...
void *thread(void *conn){
	...
	// icp
		...
		while(ptr->pc_thread_tar.size() != 0){
			toROSMsg(*(ptr->map), ptr->ros_cloud_msg);
			ptr->ros_cloud_msg.header.frame_id = "map";
			ptr->pc_map_pub.publish(ptr->ros_cloud_msg);
			...
			*(ptr->pc_target_thread) = ptr->pc_thread_tar[0];
			...
			if (ptr->count <= ptr->len){
				ptr->pc_target_thread.reset(new PointCloud<PointXYZ>());
			}
			...
			if (ptr->count > ptr->score_threshold){
...
void icp_local::process_node(const sensor_msgs::PointCloud2 msg){
	...
	*pc_target = *map;
	...
	pass.setFilterLimits (z_sum - 10, z_sum + 3);
	pass.filter (*pc_target);
	...
	// pass.setFilterFieldName ("z");
	// pass.setFilterLimits (z_sum + 0, z_sum + 10);		// -1 ,2 
