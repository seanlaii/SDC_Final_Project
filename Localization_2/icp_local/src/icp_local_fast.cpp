...
icp_local::icp_local(){
	...
	seg.setDistanceThreshold (0.1); 	// 0.5
	...
	downsample_map.setLeafSize (1.2, 1.2, 1.2);
	...
	// }
	get_initial_guess();
	pc_sub = nh.subscribe("/points_raw", 1000, &icp_local::process_node, this);
		...
		while(ptr->pc_thread_tar.size() != 0){
			...
			*(ptr->pc_target_thread) = ptr->pc_thread_tar[0];
			...
			if (ptr->thres_count != 0){
				...
				score = ptr->icp.getFitnessScore();
				cout << " score: " << score << endl;

				// if ( ptr->icp.getFitnessScore() < ptr->score_thres && ptr->icp_set){
				// 	ptr->icp_set = false;
				...
				// }
				// else if(ptr->icp.getFitnessScore() >= ptr->score_thres && !ptr->icp_set){
				// 	ptr->icp_set = true;
			...
			if (ptr->thres_count > ptr->score_threshold && ptr->thres_count <= ptr->cons){
			...
			else if (ptr->thres_count % ptr->_co == 0){
				p_odom.header = ptr->header_list[0];
				p_odom.pose.pose.position.x = ptr->initial_guess(0,3);
				p_odom.pose.pose.position.y = ptr->initial_guess(1,3);
				p_odom.pose.pose.position.z = ptr->initial_guess(2,3);
...
void icp_local::process_node(const sensor_msgs::PointCloud2 msg){
	...
	*pc_target = *map;
	...
	pass.setFilterLimits (initial_guess(1,3) - 180,initial_guess(1,3) + 180);
	pass.filter (*pc_target);
	pass.setFilterFieldName ("x");
	pass.setFilterLimits (initial_guess(0,3) - 180,initial_guess(0,3) + 180);
	pass.filter (*pc_target);
